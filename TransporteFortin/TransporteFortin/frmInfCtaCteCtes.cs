...
namespace TransporteFortin
{
    public partial class frmInfCtaCteCtes : Form
    {
        ...
        private void frmInfCtaCteCtes_Load(object sender, EventArgs e)
        {
            ...
            if (ch == 0)
            {
                CtaCteClientesBindingSource.DataSource = oa.leerDatos("select cli.cliente as Clientes, date_format(f.fecha,'%d/%m/%Y') as Fecha, c.descripcion as Conceptos, concat(ifnull(f.descripcion,''), ' Nro ', case when o.nrocarga is not null then concat(ifnull(cast(o.nrocarga as char),''), '- Ordenes de Carga') else concat(ifnull(cast(r.nro as char),''), '- Recibo') end) as Descripcion, f.ptoventa, debe as Debe, haber as Haber, case when o.nrocarga is not null then concat(cast(o.nrocarga as char), '- Ordenes de Carga') else concat(cast(r.nro as char), '- Recibo') end  as Clientes from ctacteclientes f inner join conceptos c on f.idconceptos = c.codigo left join ordenescarga o on f.idordenescarga = o.idordenescarga left join recibos r on r.idrecibos = f.idrecibos left join clientes cli on f.idclientes = cli.idclientes where f.idclientes = '" + idcli + "'");
