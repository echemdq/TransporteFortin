        ...
        private void frmInfCtaCteProv_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (DateTime.TryParse(de, out desde) && DateTime.TryParse(ha, out hasta))
                {
                    if (desde <= hasta)
                    {
                        CtaCteClientesBindingSource.DataSource = oa.leerDatos("select cli.proveedor as Clientes, date_format(f.fecha,'%d/%m/%Y') as Fecha, c.descripcion as Conceptos, f.ptoventa, debe as Debe, haber as Haber, concat(f.descripcion, ' Nro ', case when o.nro is not null then concat(cast(o.nro as char), '- Ordenes de Combustible') else case when r.nro is not null and r.tipo = 0 then concat(cast(r.nro as char), '- Recibo') else concat(cast(r.nro as char), '- Orden de Pago') end end) as Descripcion from ctacteproveedores f inner join conceptos c on f.idconceptos = c.codigo left join ordenescombustible o on f.idordenescombustible = o.idordenescombustible left join recibos r on r.idrecibos = f.idrecibos left join proveedores cli on f.idproveedores = cli.idproveedores where f.idproveedores = '"+idcli+"' and f.fecha between '" + desde.ToString("yyyy-MM-dd") + "' and '" + hasta.ToString("yyyy-MM-dd") + "'");
                        DataTable dt = oa.leerDatos("SELECT SUM(DEBE-HABER) as saldo FROM ctacteproveedores WHERE idproveedores = '" + idcli + "'");
                        string saldo = "";
                        foreach (DataRow dr in dt.Rows)
                        {
